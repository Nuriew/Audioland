...
function Pinterest() {
  ...
  const handleDownload = async () => {
    ...
    if (!link.includes("[pinterest.com") && !link.includes ("pin.it")) {
      setError("The link is incorrect. Please enter a valid Pinterest URL.");
      setIsLoading(false);
      return;
    }

    try {
      const response = await fetch('http://173.249.60.195:5000/api/pinterest', {
        method: 'POST',
  ...
  return (
    <div className="page-container">
      <div className="card" style={{ background: '#a30000' }}>
        <h1>Pinterest MP3 Downloader</h1>
